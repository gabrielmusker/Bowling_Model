...
parameters {
  ...
  vector[Y] epsilon; //Dummy variable for random walk prior
  vector<lower=0>[Y] sigma2_delta; // variance for year effect prior
...
transformed parameters {
  ...
  vector[Y] delta; // Year effect parameter
  ...
  vector[N] f_a = (-1 * alpha2_vector) .* ((Age_squared_std) - (alpha1_vector .* (Age_std)) + (alpha1_vector .* alpha1_vector));
  
  for (i in 1:P){
    theta[i] = mu_theta[i] + sqrt(sigma2_theta[i])*theta_tilde[i];
  }
  
  delta[Y] = sqrt(sigma2_delta[Y])*epsilon[Y];
  for(l in 1:(Y-1)) {
    delta[(Y-l)] = delta[((Y+1)-l)] + sqrt(sigma2_delta[(Y-l)])*epsilon[(Y-l)];
  }
  
  lambda = exp((Players*theta) + (zeta*HA) + f_a + (Inns*nu) + (Opposition*xi) + (Year*delta) + (OppEra*omega));
